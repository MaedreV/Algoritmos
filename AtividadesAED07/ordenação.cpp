...
void bubblesort(int *array, int size) {
  ...
  for (int i = 0; i < size - 1; i++) {
    bool swapped = false; // Inicializa uma flag para verificar se houve trocas
                          // nesta passagem.
...
void merge(int *array, int *aux, int start, int mid, int finish) {
  // TO DO
  int left_size = mid - start; // apenas um auxiliar já basta
  int right_size = finish - mid + 1;
...
int partition(int *array, int start, int finish) {
  // TO DO
  int pivot = array[finish]; // Escolha o último elemento como pivô.
  int i = start - 1;
